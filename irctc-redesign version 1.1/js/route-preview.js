/* ==========================================================================
   ...
   the user sees is drawn from data.
   ...
   - Every station coordinate and every route waypoint is real data
     from github.com/anandology/railways (CC0) - see stations.js /
     routes.js for full provenance. Nothing in this file invents
     geography.
   ========================================================================== */
...
(function () {
  ...
  // A route may optionally provide a map-space trace calibrated against the
  // supplied SVG artwork. This is deliberately route data, not a second
  // coordinate system: raw geographic data remains authoritative, while the
  // trace handles the non-uniform distortion of this illustrative map.
  function getRouteMapPoints(route, reversed) {
    if (route.mapWaypoints && route.mapWaypoints.length > 1) {
      var pts = route.mapWaypoints.slice();
      if (reversed) pts.reverse();
      return pts;
    }
    var geo = reversed ? route.waypoints.slice().reverse() : route.waypoints;
    return geo.map(function (pair) {
      var p = geoToMap(pair[0], pair[1]);
      return [p.x, p.y];
    });
  }

  function getRouteEndpoint(route, station, role, reversed) {
    if (route.mapWaypoints && route.mapOrigin && route.mapDestination) {
      var pair;
      if ((!reversed && role === "origin") || (reversed && role === "destination")) {
        pair = route.mapOrigin;
      } else {
        pair = route.mapDestination;
      }
      return { x: pair[0], y: pair[1] };
    }
    return geoToMap(station.lon, station.lat);
  }

  function getRouteStopPosition(route, station, reversed) {
    if (route.mapStops && route.mapStops[station.id]) {
      var pair = route.mapStops[station.id];
      return { x: pair[0], y: pair[1] };
    }
    return geoToMap(station.lon, station.lat);
  }
  ...
  // Marker rendering
  // ------------------------------------------------------------------
  function renderStationMarker(station, kind, onEntranceComplete, positionOverride) {
    var pos = positionOverride || geoToMap(station.lon, station.lat);

    // Keep the geographic translation on a STATIC wrapper. Anime.js must
    // animate a child group instead of the translated group itself. Scaling
    // the translated SVG <g> would scale from the SVG origin (0,0), which
    // makes the marker visually jump toward the corner of the map during
    // its entrance animation. The child-group approach preserves the exact
    // calibrated endpoint position while still allowing the same animation.
    var g = svgEl("g");
    g.setAttribute("class", "map-marker map-marker--" + kind);
    g.setAttribute("transform", "translate(" + pos.x.toFixed(1) + "," + pos.y.toFixed(1) + ")");

    var animated = svgEl("g");
    animated.setAttribute("class", "map-marker__animated");
    g.appendChild(animated);

    var halo = svgEl("circle");
    halo.setAttribute("class", "map-marker__halo");
    halo.setAttribute("r", kind === "waypoint" ? 4 : 11);
    animated.appendChild(halo);

    var core = svgEl("circle");
    core.setAttribute("class", "map-marker__core");
    core.setAttribute("r", kind === "waypoint" ? 1.6 : 4);
    animated.appendChild(core);
    ...
    if (!reducedMotion && window.anime) {
      // Animate only the local child group. This keeps the marker anchored
      // at its calibrated map coordinate throughout the entire entrance.
  ...
  function renderRoute(matched, onComplete) {
    ...
    var mapPts = getRouteMapPoints(route, matched.reversed);
    var pointsAttr = mapPts.map(function (p) {
      return p[0].toFixed(1) + "," + p[1].toFixed(1);
    }).join(" ");
    ...
    (route.intermediateStops || []).forEach(function (id) {
      ...
      if (st) renderStationMarker(st, "waypoint", null, getRouteStopPosition(route, st, matched.reversed));
    });
  ...
  function renderActiveState() {
    ...
    var doRender = function () {
      ...
      if (matched) {
        ...
        var originMarker = renderStationMarker(
          state.origin,
          "origin",
          function () {
            var destMarker = renderStationMarker(
              state.destination,
              "destination",
              function () {
              ...
              getRouteEndpoint(matched.route, state.destination, "destination", matched.reversed)
            );
          },
          getRouteEndpoint(matched.route, state.origin, "origin", matched.reversed)
        );
